...
class LineaController extends AbstractController
{
    ...
    public function list(Request $request, LineaRepository $lineas, string $estado): Response
    {
        ...
        $total = $lineas->findTotalLineasByEstado($params, $estado, $unidad);
        $lineas = $lineas->findLineasByEstado($params, $estado, $unidad);

        $result = [
            'total' => empty($lineas) ? 0 : $total,
            'rows' => $lineas
        ];
    ...
    public function new(Request $request): Response
    {
        ...
        $breadcrumb = [
            ['title' => 'Lineas', 'url' => $this->generateUrl('app_tic_linea', ['estado' => 'activas'])],
            ['title' => 'Registrar']
        ];
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_tic_linea', ['estado' => 'activas']);
        }
     ...
     *      methods={"GET", "POST"})
     * @Entity("linea", expr="repository.findOneLineaById(id)")
     */
    public function edit(Request $request, Linea $linea)
    {
        $breadcrumb = [
            ['title' => 'Líneas', 'url' => $this->generateUrl('app_tic_linea', ['estado' => $linea->getIsBaja() ? 'bajas' : 'activas'])],
            ['title' => 'Modificar']
        ];
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_tic_linea', [
    ...
    public function reportPlanesVoz(Request $request, LineaRepository $lineas, Pdf $pdf): Response
    {
        ...
        $html = $this->renderView('tic/report/linea_report_planes_voz.pdf.html.twig', [
            'planes' => $lineas->findTotalesByPlanVozGroupByUnidad($unidad)
        ]);
    ...
    public function reportPlanesDatos(Request $request, LineaRepository $lineas, Pdf $pdf): Response
    {
        ...
        $html = $this->renderView('tic/report/linea_report_planes_datos.pdf.html.twig', [
            'planes' => $lineas->findTotalesByPlanDatosGroupByUnidad($unidad)
        ]);
        ...
        // return $this->render('tic/report/linea_report_planes_datos.pdf.html.twig',[
        //     'planes' => $lineas->findTotalesByPlanDatosGroupByUnidad($unidad)
        // ]);
