...
class ContactoController extends AbstractController
{
     ...
     *
     * @Route("/{id<[1-9]\d*>}/test",
     *      name="app_contacto_test",
     *      methods={"GET", "POST"}
     * )
     */
    public function test(Request $request, Contacto $contacto, EntityManagerInterface $em): Response
    {
        $form = $this->createForm(ContactoType::class, $contacto);
        $breadcrumb = [
            ['title' => 'Contactos', 'url' => $this->generateUrl('app_contacto_index')],
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $changeSet = $uow->getEntityChangeSet($contacto);

            dump($changeSet['ubicacion'][0]->getNombre());
            // dump($changeSet['observacion']);
